create table Employee(
  ...
  DepNo int,
  primary key(SSN),
  foreign key(SuperSNN) references Employee 
  );
 create table Department0(
   ...
   primary key(DepNo)  );
   alter table employee add constraint fkdep foreign key(DepNo) references DDepartment0 on delete cascade on update cascade
   alter table ddepartment0 rename to department0
   alter table department0 add constraint fkssn foreign key(MgrSSN) references Employee on delete cascade on update cascade  
 create table Dept_Locarions(
   DepNo int,
   DLocation varchar(20),
   primary key(DepNo,DLocation),
   foreign key(DepNo) references Department0 on delete cascade on update cascade);  
   alter table dept_locarions rename to dept_locations
 ...
 create table Works_On(
   ...
   alter table works_on drop constraint works_on_pno_fkey
   alter table works_on add constraint pno_fkey  foreign key(pno) references project on delete cascade on update cascade
...
INSERT INTO Department0 VALUES ('Research',5,'333445555','22-MAY-1978');
INSERT INTO department0 VALUES ('Administration', 4, '987654321', '01-JAN-85');
INSERT INTO department0 VALUES ('Headquarters', 1, '888665555', '19-JUN-71');
INSERT INTO department0 VALUES ('Software',6,'111111100','15-MAY-1999');
INSERT INTO department0 VALUES ('Hardware',7,'444444400','15-MAY-1998');
INSERT INTO department0 VALUES ('Sales',8,'555555500','01-JAN-1997');
...
select DName,min(salary) from Employee,Department0 where employee.depno=department0.depno group by Dname
select DName,avg(salary) from Employee,Department0 where employee.depno=department0.depno group by Dname having avg(salary) >40000
select count(FName) from Employee where depno = 6
select Dname,count(FName) from Employee,department0 where  FName like 'J%' and department0.depno=employee.depno group by Dname
select dname,count(PNumber) from Department0,Project where department0.depno=project.depno group by Dname  
select * from Project,Department0 where department0.depno=project.depno